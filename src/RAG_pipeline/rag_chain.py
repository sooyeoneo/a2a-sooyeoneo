...
from rag_pipeline.prompt_manager import (
    PromptManager,
    OutputParser,
...
class RAGChain:
    ...
    def process_company_query(self, company_name: str, question: str) -> Dict[str, Any]:
        ...
        try:
            ...
            vector_store = self.embedding_manager.create_vector_store(
            ...
            return self._create_error_response(
                f"처리 중 오류 발생: {str(e)}", processing_time
            )
        ...
        try:
                ...
                if similar_chunks:
                    ...
                    if self.llm:
                        try:
                            ...
                            # JSON 파싱
                            parsed_data = self.output_parser.parse_json_response(
                                response_text
                            )
            ...
            result = {
                ...
                "fields_with_data": sum(
                    1 for v in report_data.values() if v is not None
                ),
            }
            ...
            processing_time = time.time() - start_time
            return self._create_error_response(
                f"리포트 생성 중 오류: {str(e)}", processing_time
            )
